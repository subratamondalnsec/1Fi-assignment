...
const emiPlanSchema = new Schema(
  {
    tenure: {
      ...
      validate: {
        ...
        message: "EMI tenure must be a whole number of months.",
      },
    },
...
const variantSchema = new Schema(
  {
    ...
    mrp: {
      ...
      min: 0.01,
      validate: {
        validator: function validateMrp(value) {
    ...
    images: {
      ...
      validate: [
        {
          validator: function hasImage(images) {
            return images.length > 0 || remoteImageValidator(this.imageUrl);
          },
          message: "Each variant must include at least one image.",
        },
        {
          validator: (images) =>
            images.every(
              (image) =>
                typeof image === "string" && remoteImageValidator(image.trim()),
            ),
          message: "Images must contain valid remote URLs.",
        },
      ],
    },
    ...
    emiPlans: {
      ...
      default: [],
      validate: {
        validator: (plans) => plans.length > 0,
        message: "Each variant must include at least one EMI plan.",
      },
    },
  },
  { _id: true },
...
const productSchema = new Schema(
  {
    ...
    variants: {
      ...
      validate: {
        ...
        message: "A product must include at least one variant.",
      },
    },
  },
  { timestamps: true },
