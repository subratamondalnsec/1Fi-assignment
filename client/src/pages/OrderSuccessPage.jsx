...
export function OrderSuccessPage() {
  ...
  useEffect(() => {
    ...
    getOrder(orderId)
      ...
      .catch((error) => {
        if (active)
          setState({ status: "error", order: null, message: error.message });
      });
    return () => {
      active = false;
    };
  ...
  if (state.status === "error")
    return (
      <section className="rounded-xl border border-red-200 bg-red-50 p-8">
        ...
        <Link
          className="mt-5 inline-flex font-semibold text-red-900 underline"
          to="/"
        >
          Return to catalogue
        </Link>
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-6">
        ...
        <p className="mt-2 text-emerald-900">Order {order.orderNumber}</p>
      </div>
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-4">
            {order.items.map((item) => (
              <article
                className="flex gap-4 border-t border-slate-100 pt-4 first:border-t-0 first:pt-0"
                key={`${item.productId}-${item.variantId}-${item.emiPlanId}`}
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    {item.emiTenure} months at{" "}
                    {formatCurrency(item.emiMonthlyAmount)}/month
                  </p>
                  <p className="text-sm text-slate-600">
                    Cashback: {formatCurrency(item.emiCashback)}
                  </p>
                </div>
                <p className="font-semibold text-slate-950">
        ...
        <aside className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div>
              <dt className="text-slate-600">Razorpay payment ID</dt>
          ...
          <Link
            className="mt-6 inline-flex font-semibold text-indigo-600 hover:text-indigo-700"
            to="/"
          >
            Continue shopping
          </Link>
