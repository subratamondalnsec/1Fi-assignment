...
export function EmiPlans({ plans, selectedPlan, onSelect }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{plans.map((plan, index) => <EmiCard isRecommended={recommendedPlan?.id === plan.id} isSelected={selectedPlan?.id === plan.id} key={plan.id || `${plan.tenure}-${plan.monthlyAmount}-${index}`} onSelect={onSelect} plan={plan} />)}</div>;
}
